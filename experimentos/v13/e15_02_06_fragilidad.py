"""E15-02 punto 6: fragilidad con JS (W=100). Jackknife quitando 5/10/20 batches al azar (300 rep c/u):
distribucion de t (%t>1.645 y %t>2.33). Leave-one-block-out (bloques cronologicos de 20)."""
import sys, time
...
for k in (5, 10, 20):
    ...
    for _ in range(300):
        ...
        sc = L13.wf(X[keep], C[keep], y[keep], 100, modo="js")
    ...
    pct1 = 100 * np.mean(ts > 1.645); pct2 = 100 * np.mean(ts > 2.33)
    filas.append({"quitando": k, "t_P5": p5, "t_P50": p50, "t_P95": p95, "pend_P5": s5, "pend_P50": s50, "pend_P95": s95,
                  "pct_t_gt_1.645": pct1, "pct_t_gt_2.33": pct2})
    print(f"jackknife-{k}: t P5 {p5:.2f} P50 {p50:.2f} P95 {p95:.2f} | pend P50 {s50:.2f} | %t>1.645 {pct1:.0f} %t>2.33 {pct2:.0f}  [{time.time()-t0:.0f}s]")

pd.DataFrame(filas).to_csv("experimentos/v13/e15_02_06_jackknife.csv", index=False)

# leave-one-block-out (bloques cronologicos de 20)
bl20 = L13.bloques(n, 20, seed=0)
obs_full = W7.stats_final(L13.wf(X, C, y, 100, modo="js"), y, C)
filas_b = []
...
outb.to_csv("experimentos/v13/e15_02_06_loo_bloques.csv", index=False)
