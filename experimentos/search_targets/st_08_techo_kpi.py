...
Salidas: st_08_fiabilidad.csv, st_08_techo_modelos.csv, st_08_log.txt, figs/st_08_*.png
"""
...
pd.DataFrame([fila]).T.to_csv(AQUI / "st_08_fiabilidad.csv")
...
for yname, y in ys.items():
    ...
    ok = yd.notna().values
    for mname in ["Ridge", "ElasticNet", "HGB"]:
        r2s, rhos = [], []
        preds = np.full(ok.sum(), np.nan)
        rkf = RepeatedKFold(n_splits=5, n_repeats=3, random_state=42)
        Xdo, ydo = Xd[ok], yd[ok]
        oof_all = np.zeros((3, len(ydo)))
...
res = pd.DataFrame(filas); res.to_csv(AQUI / "st_08_techo_modelos.csv", index=False)
...
imp.head(40).to_csv(AQUI / "st_08_importancia_hgb.csv")
...
plt.xticks(rotation=15); plt.tight_layout(); fig.savefig(AQUI / "figs" / "st_08_techo.png", dpi=120)
log(f"tiempo {time.time()-t0:.0f}s")
(AQUI / "st_08_log.txt").write_text("\n".join(LOG), encoding="utf-8")
