...
Etapas (argumento CLI):
  validacion      -> e7_05_tabla_validacion_v5_lanza.csv, e7_05_efectos_control_v5_lanza.csv, e7_05_pred_<fase>_<clave>.csv,
                     e7_05_curvas_respuesta_v5_lanza.csv
...
SUF = "_lanza"; mp5.activar_variante_lanza(); print("variante", mp5.VARIANTE_ACTIVA, mp5.PALANCAS_V5[("Reducción", "sn_dep")])
PESOS = None if W_SOP is None else {"w_soporte": W_SOP}

if etapa == "validacion":
    tabla = mp5.tabla_validacion_v5(df); tabla.to_csv(SALIDA / "e7_05_tabla_validacion_v5_lanza.csv", index=False)
    log("\n", tabla.round(3).to_string())
    sistema = mp5.entrenar_sistema_v5(df, n_boot=300)
    ef = mp5.tabla_efectos_control_v5(sistema); ef.to_csv(SALIDA / "e7_05_efectos_control_v5_lanza.csv", index=False)
    ...
    for fase in mp5.FASES:
        for clave in mp5.TARGETS_V5[fase]:
            ...
            p.to_csv(SALIDA / f"e7_05_pred_{fase}_{clave}_lanza.csv", index=False)
    ...
    for orden in (1, 3, 5):
        ...
        for pal in mp5.ACCIONES_OPTIMIZABLES_V5["Fusión"]:
            ...
            curvas.append(c)
    cur = pd.concat(curvas, ignore_index=True); cur.to_csv(SALIDA / "e7_05_curvas_respuesta_v5_lanza.csv", index=False)
